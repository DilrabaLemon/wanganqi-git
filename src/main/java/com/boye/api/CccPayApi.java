...
import com.boye.common.http.pay.CccPayParamBean;
import com.boye.common.http.pay.CccPayResultBean;
...
public class CccPayApi {

	private static Logger logger = LoggerFactory.getLogger(CccPayApi.class);

    public static final String PAYURL = "http://pay.cccpay.cc/gateway/order";
...
//    public static final String BALANCEURL = "https://8oep1k.apolo-pay.com/amountquery";
    ...
    public static Map<String, Object> getQrCode(CccPayParamBean payParam) {
    	Map<String, Object> result = new HashMap<String, Object>();
		Gson gson = new Gson();
		Map<String, Object> paramMap = new HashMap<String, Object>();
		paramMap.put("params", gson.toJson(payParam.hasSignParamMap()));
        String rest = JsonHttpClientUtils.httpPost(PAYURL, gson.toJson(paramMap), "UTF-8"); 	//返回json串
        rest = rest.substring(rest.indexOf("\"{") + 1, rest.indexOf("}\"") + 1);
        logger.info(rest);
        try {
        	CccPayResultBean reMsg = gson.fromJson(rest, CccPayResultBean.class);
            if ("0".equals(reMsg.getCode())) {
            	String url = null;
            	if (reMsg.getQrcode() != null && !reMsg.getQrcode().equals("")) {
            		url = reMsg.getQrcode();
            	} else {
            		url = reMsg.getPay_url();
            	}
            	result.put("code", 1);
            	result.put("data", url);
            	result.put("msg","获取成功");
...
//	}
	...
	public static Map<String, Object> getQrCode(AuthenticationInfo authentication, PaymentAccount usePayment, PassagewayInfo passagewayInfo) { 
		...
		CccPayParamBean payParam = new CccPayParamBean();
		
		payParam.setAmount(new BigDecimal(authentication.getPayment()).multiply(BigDecimal.valueOf(100)));
		payParam.setMchid(usePayment.getAccount_number());
		payParam.setNoncestr(new Date().getTime() + "");
		...
		payParam.setTrade_out_no(authentication.getPlatform_order_number());
