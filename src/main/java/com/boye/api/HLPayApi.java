...
import com.boye.common.http.pay.HLPayParamBean;
...
import com.boye.common.http.query.HLPayQueryParamBean;
import com.boye.common.http.query.HLPayQueryResultBean;
import com.boye.common.utils.JsonHttpClientUtils;
import com.google.gson.Gson;

public class HLPayApi {

	private static Logger logger = LoggerFactory.getLogger(HLPayApi.class);

    public static final String PAYURL = "http://103.60.220.140/huilian-api/services/onlineBank/onlineBank_v1";
...
//
    public static final String QUERYURL = "http://103.60.220.140/huilian-api/services/order/orderQuery";
//    
//    public static final String BALANCEURL = "https://8oep1k.apolo-pay.com/amountquery";

    public static Map<String, Object> getQrCode(HLPayParamBean payParam) {
    	Map<String, Object> result = new HashMap<String, Object>();
//		Gson gson = new Gson();
        String rest = JsonHttpClientUtils.httpPostform(PAYURL, payParam.hasSignParamMap(), "UTF-8"); 	//返回json串
        logger.info(rest);
//        HJPayResultBean reMsg = gson.fromJson(rest, HJPayResultBean.class);
//        if ("0".equals(reMsg.getCode()) && reMsg.getData() != null) {
        	result.put("code", 1);
...
//	}
	...
	public static SearchPayBo queryPayInfo(HLPayQueryParamBean queryParam) {
		Gson gson = new Gson();
	    String rest = JsonHttpClientUtils.httpPost(QUERYURL, gson.toJson(queryParam.hasSignParamMap()), "UTF-8"); 	//返回json串
	    HLPayQueryResultBean queryBean = null;
	    SearchPayBo searchPayBo = new SearchPayBo();
	    try {
	    	queryBean = (HLPayQueryResultBean) gson.fromJson(rest, HLPayQueryResultBean.class);
	    	logger.info("获取成功：" + rest);
	    	if ("00".equals(queryBean.getRsp_code()) && queryBean.getState() != null) {
	    		if ("0".equals(queryBean.getState())) {
	    			searchPayBo.setState(SearchPayStatus.GETSUCCESS);
	    			DecimalFormat df = new DecimalFormat("#0.00");
	    			BigDecimal money = new BigDecimal(queryBean.getOrder_amt());
	    			money = money.divide(BigDecimal.valueOf(100));
		    		searchPayBo.setMoney(df.format(money));
	    		} else if ("2".equals(queryBean.getState())) {
	    	...
	    	searchPayBo.setAccount_number(queryParam.getMerchant_id());
	    	searchPayBo.setMsg(queryBean.getRsp_msg());
	...
	public static Map<String, Object> getQrCode(QuickAuthenticationInfo authentication, PaymentAccount usePayment, PassagewayInfo passagewayInfo) { 
		...
		HLPayParamBean payParam = new HLPayParamBean();
		
		payParam.setBank_code(authentication.getCard_code());
		payParam.setBg_url(passagewayInfo.getNotify_url());
		payParam.setBiz_code("1005");
		payParam.setCard_type("1");
		payParam.setExtra(authentication.getOrder_number());
		payParam.setMerchant_id(usePayment.getAccount_number());
		payParam.setOrder_amt(new BigDecimal(authentication.getPayment()));
		payParam.setOrder_id(authentication.getPlatform_order_number());
