...
public class VPayApi {

	private static Logger logger = LoggerFactory.getLogger(VPayApi.class);

    public static final String PAYURL = "http://47.244.112.63:3020/api/pay/create_order";
...
//
    public static final String QUERYURL = "http://47.244.112.63:3020/api/pay/query_order";
//    
//    public static final String BALANCEURL = "https://8oep1k.apolo-pay.com/amountquery";

    public static Map<String, Object> getQrCode(BePayParamBean payParam) {
    	Map<String, Object> result = new HashMap<String, Object>();
		Gson gson = new Gson();
		Map<String, Object> paramMap = new HashMap<String, Object>();
		paramMap.put("params", gson.toJson(payParam.hasSignParamMap()));
        String rest = JsonHttpClientUtils.httpPostform(PAYURL, paramMap, "UTF-8"); 	//返回json串
        logger.info(rest);
        try {
        	BePayResultBean reMsg = gson.fromJson(rest, BePayResultBean.class);
            if ("SUCCESS".equals(reMsg.getRetCode()) && reMsg.getPayParams() != null) {
            	result.put("code", 1);
...
//	}
	...
	public static Map<String, Object> getQrCode(AuthenticationInfo authentication, PaymentAccount usePayment, PassagewayInfo passagewayInfo) { 
		...
		BePayParamBean payParam = new BePayParamBean();
		...
		payParam.setMchOrderNo(authentication.getPlatform_order_number());
		payParam.setNotifyUrl(passagewayInfo.getNotify_url());
		payParam.setProductId("8009");
