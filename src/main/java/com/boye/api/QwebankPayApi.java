...
import com.boye.common.http.pay.QwebankTokenParamBean;
import com.boye.common.http.pay.QwebankTokenResultBean;
...
public class QwebankPayApi {

	private static Logger logger = LoggerFactory.getLogger(QwebankPayApi.class);
	
	public static final String GETACCESSTOKEN = "http://api.qwebank.top/open/v1/getAccessToken/merchant";

    public static final String PAYURL = "http://api.qwebank.top/open/v1/order/alipayWapPay";
    ...
    public static final String QUERYURL = "http://api.qwebank.top/open/v1/order/getByCustomerNo";
    
    public static final String BALANCEURL = "https://8oep1k.apolo-pay.com/amountquery";
    
    public static String getAccessToken(QwebankTokenParamBean tokenParam) {
		Gson gson = new Gson();
		String json = gson.toJson(tokenParam.hasSignParamMap());
		System.out.println(json);
        String rest = JsonHttpClientUtils.httpsPost(GETACCESSTOKEN, json, "UTF-8"); 	//返回json串
        QwebankTokenResultBean reMsg = gson.fromJson(rest, QwebankTokenResultBean.class);
        if (reMsg.isSuccess() && reMsg.getValue() != null) {
        	return reMsg.getValue().getAccessToken();
        } else {
        	return null;
        }
    }
