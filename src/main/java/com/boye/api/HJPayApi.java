...
public class HJPayApi {

	private static Logger logger = LoggerFactory.getLogger(HJPayApi.class);

    public static final String PAYURL = "http://www.hjjzf.com/api/payapi/scanpay";
...
//
    public static final String QUERYURL = "http://www.hjjzf.com/api/payapi/queryorder";
//    
//    public static final String BALANCEURL = "https://8oep1k.apolo-pay.com/amountquery";

    public static Map<String, Object> getQrCode(HJPayParamBean payParam) {
        ...
        if ("0".equals(reMsg.getCode()) && reMsg.getData() != null) {
        	result.put("code", 1);
        	result.put("data", reMsg.getData().getQr_code_url());
        } else {
        	result.put("code", 2);
        	result.put("data", "");
        }
...
//	}
	...
	public static Map<String, Object> getQrCode(AuthenticationInfo authentication, PaymentAccount usePayment, PassagewayInfo passagewayInfo) { 
		...
		HJPayParamBean payParam = new HJPayParamBean();
		
		payParam.setBrower_url(passagewayInfo.getReturn_url());
		payParam.setCash(new BigDecimal(authentication.getPayment()));
		payParam.setMerchant_number(usePayment.getAccount_number());
		payParam.setOrder_id(authentication.getPlatform_order_number());
//		SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		payParam.setOrder_time(new Date().getTime()/1000 + "");
		payParam.setPay_type(passagewayInfo.getPay_type());
		payParam.setServer_url(passagewayInfo.getNotify_url());
		payParam.setVersion("3");
