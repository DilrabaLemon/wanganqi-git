...
import com.boye.common.http.pay.DingEPayParamBean;
import com.boye.common.http.pay.DingEPayResultBean;
...
public class DingEPayApi {

	private static Logger logger = LoggerFactory.getLogger(DingEPayApi.class);

    public static final String PAYURL = "http://47.105.221.115:12202/v1.0/GetOrderByParvalue.ashx";
...
//    public static final String BALANCEURL = "https://8oep1k.apolo-pay.com/amountquery";
	...
	public static Map<String, Object> getQrCode(DingEPayParamBean payParam) {
    	Map<String, Object> result = new HashMap<String, Object>();
		Gson gson = new Gson();
        String rest = JsonHttpClientUtils.httpPostform(PAYURL, payParam.hasSignParamMap(), "UTF-8"); 	//返回json串
        logger.info(rest);
        try {
        	Map<String, Object> reMsg = gson.fromJson(rest, Map.class);
            if ("10014".equals(new BigDecimal(reMsg.get("Code").toString()).intValue() + "")) {
            	result.put("code", 1);
            	Map<String, Object> dataBody = (Map<String, Object>) reMsg.get("Data");
            	String linkMapString = dataBody.get("WechatPaymentInfo").toString();
            	linkMapString = linkMapString.substring(1, linkMapString.length() - 1);
            	DingEPayResultBean link = gson.fromJson(linkMapString, DingEPayResultBean.class);
            	result.put("data", link.getLink());
            	result.put("orderId", dataBody.get("OrderId"));
            	result.put("msg","获取成功");
            } else {
            	result.put("code", 2);
            	result.put("data", "");
            	result.put("msg", reMsg.get("Message"));
            }
        } catch(Exception e) {
        	e.printStackTrace();
        	result.put("code", 2);
...
//	}
	...
	public static Map<String, Object> getQrCode(AuthenticationInfo authentication, PaymentAccount usePayment, PassagewayInfo passagewayInfo) { 
		...
		DingEPayParamBean payParam = new DingEPayParamBean();
		payParam.setParvalue(new BigDecimal(authentication.getPayment()));
		payParam.setCallbackUrl(passagewayInfo.getNotify_url());
		payParam.setTimestamp(new Date().getTime());
		payParam.setUserId(usePayment.getAccount_number());
		
		payParam.setKey(usePayment.getAccount_key());
		payParam.setSign(payParam.generateSign());
		Map<String, Object> result = getQrCode(payParam);
		if (result.get("code").equals(1)) {
			authentication.setPlatform_order_number(result.get("orderId").toString());
